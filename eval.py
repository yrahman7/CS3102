...
def EQUAL_3(i, j):
    ...
    pre_temp0 = XOR(i[0], j[0])
    temp0 = NAND(pre_temp0, pre_temp0)
    pre_temp1 = XOR(i[1], j[1])
    temp1 = NAND(pre_temp1, pre_temp1)
    pre_temp2 = XOR(i[2], j[2])
    temp2 = NAND(pre_temp2, pre_temp2)

    temp_and = AND(temp0, temp1)
    return AND(temp_and, temp2)
...
def UPDATE_7(T, b, i):
    ...
    a0 = EQUAL_3("000", i)
    c0 = LOOKUP_3(T[0], T[1], T[2], T[3], T[4], T[5], T[6], '0', '0', '0', '0')
    t0 = IF(a0, b, c0)

    a1 = EQUAL_3("001", i)
    c1 = LOOKUP_3(T[0], T[1], T[2], T[3], T[4], T[5], T[6], '0', '0', '0', '1')
    t1 = IF(a1, b, c1)

    a2 = EQUAL_3("010", i)
    c2 = LOOKUP_3(T[0], T[1], T[2], T[3], T[4], T[5], T[6], '0', '0', '1', '0')
    t2 = IF(a2, b, c2)

    a3 = EQUAL_3("011", i)
    c3 = LOOKUP_3(T[0], T[1], T[2], T[3], T[4], T[5], T[6], '0', '0', '1', '1')
    t3 = IF(a3, b, c3)

    a4 = EQUAL_3("100", i)
    c4 = LOOKUP_3(T[0], T[1], T[2], T[3], T[4], T[5], T[6], '0', '1', '0', '0')
    t4 = IF(a4, b, c4)

    a5 = EQUAL_3("101", i)
    c5 = LOOKUP_3(T[0], T[1], T[2], T[3], T[4], T[5], T[6], '0', '1', '0', '1')
    t5 = IF(a5, b, c5)

    a6 = EQUAL_3("110", i)
    c6 = LOOKUP_3(T[0], T[1], T[2], T[3], T[4], T[5], T[6], '0', '1', '1', '0')
    t6 = IF(a6, b, c6)
...
def EVAL_3_7_4_1(program, input_bits):
    ...
    a3 = GET_7(T, program[3:6])
    b3 = GET_7(T, program[6:9])
    c3 = NAND(a3, b3)
    T = UPDATE_7(T, c3, program[0:3])

    a4 = GET_7(T, program[12:15])
    b4 = GET_7(T, program[15:18])
    c4 = NAND(a4, b4)
    T = UPDATE_7(T, c4, program[9:12])

    a5 = GET_7(T, program[21:24])
    b5 = GET_7(T, program[24:27])
    c5 = NAND(a5, b5)
    T = UPDATE_7(T, c5, program[18:21])

    a6 = GET_7(T, program[30:33])
    b6 = GET_7(T, program[33:])
    c6 = NAND(a6, b6)
    T = UPDATE_7(T, c6, program[27:30])
...
or_program = [(3, 0, 0), (4, 1, 1), (5, 0, 0), (6, 3, 4)]
